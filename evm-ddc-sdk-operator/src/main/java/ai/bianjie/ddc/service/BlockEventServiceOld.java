 ...
 */
@Slf4j
public class BlockEventServiceOld extends BaseService {
    ExecutorService executorService;

    public BlockEventServiceOld() {
        int poolSize = 5;
    ...
    public BlockEventBean getBlockEvent(BigInteger blockNumber) throws IOException, ExecutionException, InterruptedException {
        ...
        List<Future<ArrayList<BaseEventResponse>>> futureList = new LinkedList<>();
        txs.forEach(tx -> {
            CallTest ct = new CallTest(tx);
            Future<ArrayList<BaseEventResponse>> submit = executorService.submit(ct);
            futureList.add(submit);
        });
        for (Future<ArrayList<BaseEventResponse>> f : futureList) {
            //将汇总好的结果进行轮询，判断任务是否执行完成，确保每个任务执行完成后将结果添加到结果集中
            while (true) {
                if (f.isDone() && !f.isCancelled()) {
                    ArrayList<BaseEventResponse> baseEventResponses = f.get();
                    arrayList.addAll(baseEventResponses);
                    break;
                }
            }
        }
...
//        }
    ...
    static class CallTest implements Callable<ArrayList<BaseEventResponse>> {
        private EthBlock.TransactionObject tx;

        public CallTest(EthBlock.TransactionResult transaction) {
            this.tx = (EthBlock.TransactionObject) transaction.get();
        }

        @Override
        public ArrayList<BaseEventResponse> call() throws Exception {
            String hash = tx.get().getHash();
            ArrayList<BaseEventResponse> arr = null;
            boolean ok = true;
            int count = 0;
            while (ok) {
                ...
                try {
                    arr = BlockEventServiceOld.analyzeEventsByTxHash(hash);
                    Thread.sleep(300);
                } catch (Exception e) {
                    count++;
                    continue;
