...
import ai.bianjie.ddc.constant.Armored;
import ai.bianjie.ddc.constant.Constant;
import ai.bianjie.ddc.crypto.ArmoredInputStream;
import ai.bianjie.ddc.crypto.ArmoredOutputStream;
import ai.bianjie.ddc.crypto.BCryptImpl;
import ai.bianjie.ddc.exception.IritaSDKException;
import ai.bianjie.ddc.util.Bip44Utils;
import ai.bianjie.ddc.util.HashUtils;
...
public abstract class KeyManager implements Key, MultiKey {
    ...
    public void recover(InputStream keystore, String password) {
        try {
            ...
            String realSaltString = BCryptImpl.encode_base64(realSaltByte, 16);
            String salt = Armored.PREFIX_SALT + realSaltString;
    ...
    public String export(String password, byte[] priKey) {
        ...
        String salt = BCrypt.gensalt(Armored.LOG_ROUNDS);
        String keyHash = BCrypt.hashpw(password, salt);
        byte[] keyHashByte = keyHash.getBytes(StandardCharsets.UTF_8);
        byte[] keyHashSha256 = HashUtils.sha256(keyHashByte);
        ...
        String realSaltString = salt.substring(Armored.REAL_SALT_BEGIN_POS, Armored.REAL_SALT_BEGIN_POS + Armored.REAL_SALT_BASE64_LEN);
        byte[] realSaltByte = BCryptImpl.decode_base64(realSaltString, 16);
