...
import ai.bianjie.ddc.model.*;
import ai.bianjie.ddc.model.tx.*;
import ai.bianjie.ddc.util.*;
import com.google.protobuf.Any;
import com.google.protobuf.GeneratedMessageV3;
import ai.bianjie.ddc.config.ClientConfig;
import ai.bianjie.ddc.config.OpbConfig;
import ai.bianjie.ddc.constant.EventType;
import ai.bianjie.ddc.constant.TmTypes;
import ai.bianjie.ddc.constant.TxStatus;
import ai.bianjie.ddc.constant.enums.BroadcastMode;
import ai.bianjie.ddc.exception.IritaSDKException;
import ai.bianjie.ddc.exception.UnknownMsgException;
import ai.bianjie.ddc.function.EventHandler;
import ai.bianjie.ddc.model.Result;
import ai.bianjie.ddc.model.block.BlockResult;
import ai.bianjie.ddc.model.block.ResultBlock;
import ai.bianjie.ddc.model.block.ResultBlockResults;
import ai.bianjie.ddc.model.block.ResultBlockRpc;
...
public class RpcClient implements WsEvent {
    ...
    public ResultQueryTx parseResultQueryTx(ai.bianjie.ddc.model.tx.Result result) throws IOException, NoSuchMethodException, IllegalAccessException, InvocationTargetException {
        if (result == null) {
    ...
    public ResultSearchTxs queryTxs(EventQueryBuilder builder, int page, int size) throws IOException, NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        if (txsRpc.getResult().getTxs().size() > 0) {
            ...
            for (ai.bianjie.ddc.model.tx.Result result : txsRpc.getResult().getTxs()) {
